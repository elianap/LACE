...
class ProcessedDatasetTrainTest:
    ...
    def processTrainTestDataset(
        ...
        self.min_max_scaler_cols = list(
            self.train.drop(self.categorical_features, axis=1).columns
        )
    ...
    def getLabelEncodedNames(self):
        return list(
            zip(
                self.categorical_features,
                list(map(list, self.categorical_names_LE.values())),
            )
        )

    def getMappingOHColumnsTupleAttrValue(self):
        return {
            f"{k}_{v}": (k, v)
            for k, values in self.getLabelEncodedNames()
            for v in values
        }
...
def getCategoricalAndContinuous(df, feature_names):
    ...
    # MODIFIED
    continuos_features = [
        attr for attr in feature_names if attr not in categorical_features
    ]
    continuos_features_pos = [
