...
class ProcessedDataset:
    def __init__(self, df, meta_col=None, dataset_name=""):
        self.df = df
        ...
        self.predict_fn_OH = None
        self.predict_fn_class = None
        self.predict_fn_class_OH = None
        self.clf = None
    ...
    def processDataset(
        ...
        # FROM discr_par={"round_v": 0, "bins": 4, "strategy": "quantile"},
        round_v=0,
        bins=4,
        strategy="quantile",
        verbose=False,
        ...
        feature_names = df.columns.drop(cols_to_drop)
        (
            categorical_features,
            categorical_features_pos,
            continuos_features,
            continuos_features_pos,
        ) = getCategoricalAndContinuous(df, feature_names)
        labels, class_names, le = getClassLabelEncoding(df[class_col_name].values)
        # TODO vs overall
        data_LE, categorical_names_LE, encoders = getLabelEncodingMapping(
            df, feature_names, categorical_features_pos
        )
        train, test, labels_train, labels_test = SplitTrainTest(
            data_LE, feature_names, labels, all_data=all_data
        )
        ...
        attributes = getAttributes(
            df_cols, featureMasking=featureMasking, ignoreCols=ignoreCols
        )
        from src.dataset_neigh_Z import Dataset
        from copy import deepcopy
        ...
        _, _, encoders_discr = getLabelEncodingMapping(
            data_X_discretized,
            data_X_discretized.columns,
            range(0, len(data_X_discretized.columns)),
        )
        self.d_train = Dataset(
            train_orig.drop(columns=meta_col)  # .values #MODIFIED ELIANA 12/4
            if meta_col != None
            else train_orig,  # .values, #MODIFIED ELIANA 12/4
            deepcopy(attributes),
            column_encoders=all_encoders,
            featureMasking=True,
            discreteDataset=data_X_discretized.loc[train.index],
            encoders_Z=encoders_discr,
            encoder_nn=encoder_t,
            verbose=verbose,
        )
        self.d_explain = Dataset(
            test_orig.drop(columns=meta_col)  # .values #MODIFIED ELIANA 12/4
            if meta_col != None
            else test_orig,  # .values, #MODIFIED ELIANA 12/4
            deepcopy(attributes),
            column_encoders=all_encoders,
            featureMasking=True,
        ...
        self.predict_fn_OH = clf.predict_proba
        self.predict_fn_class_OH = clf.predict
