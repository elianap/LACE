...
class LACE_explainer:
    ...
    def __init__(
        ...
        self.nbrs = sklearn.neighbors.NearestNeighbors(
            n_neighbors=len(self.train_dataset),
            # metric="euclidean",
            # algorithm="auto",
            # metric_params=None,
        ).fit(self.train_dataset.X_numpy_NN())
    ...
    def explain_instance(
        ...
        explanation : dict
            ...
            ::

                {
                    ...
                    'instance':
                        ...
                        dtype: object,
                    'target_class': 'mammal',
                    'errors': {33:0.00864197530864197}, #UPDATE
            ...
            * ``target_class``: The name of the class for which the prediction
              is explained.
            * ``errors``: A dictionary of all the approximation errors calculated
              at every step of the algorithm, key=K.
        ...
        for k in range(self.starting_K, self.max_K, self.K):
            ...
            local_rules, error, pi = self._compute_lace_step(
                ...
                verbose=verbose,
                all_rules=all_rules,
                specialistic_rules=specialistic_rules,
            )
    ...
    def _compute_lace_step(
        ...
        l3clf = L3Classifier(
            min_sup=self.min_sup, specialistic_rules=specialistic_rules
        )
        if featureMasking:
    ...
    def computeGlobalExplanation(self, d_explain, n_to_explain=5, verbose=False):
        featureMasking = True
        class_column_name = self.train_dataset.class_column_name()
        attributes_pred_dif = {}
        attributes_values_pred_dif = {}
        rules_pred_dif = {}

        from src.global_utils import (
            updateKeyListDictionary,
            addTotalColumns,
            computeGlobalAverage,
        )
        import random

        ids_to_explain = random.sample(list(d_explain._encoded_df.index), n_to_explain)
        for id_i in ids_to_explain:
            if verbose:
                print(id_i, end=" ")
            ...
            explanation_fm = self.explain_instance(
                instance,
                self.getDecodedPredictedClass(instance),
                featureMasking=featureMasking,
            ...
            if verbose:
                print(id_i)
                print(instance)
                print(self.getDecodedPredictedClass(instance))

            # fig_lace, _=explanation_fm.plotExplanation(showRuleKey=False, retFig=True)
            # explanation_fm.local_rules.getDataframeRule()

            discretizedInstance_vals = explanation_fm.discretizedInstance
            if class_column_name in discretizedInstance_vals:
                discretizedInstance_vals = discretizedInstance_vals.drop("class")
            attributes = list(discretizedInstance_vals.index)
            attrs_values = [f"{k}={v}" for k, v in (discretizedInstance_vals.items())]
            rules_exp = explanation_fm.local_rules.getDictHRRules()

            if explanation_fm.target_class not in attributes_pred_dif:
                attributes_pred_dif[explanation_fm.target_class] = {}
            attributes_dict = attributes_pred_dif[explanation_fm.target_class]
            updateKeyListDictionary(
                attributes, explanation_fm.diff_single, attributes_dict
            )

            if explanation_fm.target_class not in attributes_values_pred_dif:
                attributes_values_pred_dif[explanation_fm.target_class] = {}
            attributes_values_dict = attributes_values_pred_dif[
                explanation_fm.target_class
            ]
            updateKeyListDictionary(
                attrs_values, explanation_fm.diff_single, attributes_values_dict
            )

            if explanation_fm.target_class not in rules_pred_dif:
                rules_pred_dif[explanation_fm.target_class] = {}
            rules_exp_dict = rules_pred_dif[explanation_fm.target_class]
            updateKeyListDictionary(
                list(rules_exp.keys()), list(rules_exp.values()), rules_exp_dict
            )

        addTotalColumns(attributes_pred_dif)
        addTotalColumns(attributes_values_pred_dif)
        addTotalColumns(rules_pred_dif)

        attributes_pred_dif_avg = computeGlobalAverage(attributes_pred_dif)
        attributes_values_pred_dif_avg = computeGlobalAverage(
            attributes_values_pred_dif
        )
        rules_pred_dif_avg = computeGlobalAverage(rules_pred_dif)

        infos = explanation_fm.infos
        from src.global_explanation import GlobalExplanation

        global_exp = GlobalExplanation(
            attributes_pred_dif_avg,
            attributes_values_pred_dif_avg,
            rules_pred_dif_avg,
            classes_names=d_explain.class_values(),
            infos=infos,
        )
        return global_exp
...
def _create_locality_and_get_rules_NEW(
    ...
    printInfo=True,
    all_rules=False,
    checkStableColumns=True,
    # TODO IMPORTANT CHANGE 28/04 --> move to no change?
    stableColumns_threshold=0.1,  # stableColumns_threshold=0.25,
):

    # IMPORTANT CHANGE LOCALITY DEFINITION
    # If %columns is constant: increase the locality
    # The problem is that we do not provide enough information to the local model
    ...
    # TODO:
    # return if all of the same class
    columns_n = X_neighbors.columns.to_list()
    if printInfo:
        unique_vals = list(X_neighbors.columns[X_neighbors.nunique() <= 1])
        if verbose:
            print(
                f"Unique columns with k={k}: {unique_vals}, {len(unique_vals)/len(columns_n)}"
            )

        if checkStableColumns:
            if len(unique_vals) / len(columns_n) >= stableColumns_threshold:
                return local_rules
