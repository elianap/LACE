...
class Dataset:
    ...
    def __init__(
        ...
        # self._decoded_df = pd.DataFrame(data.values, index=data.index)
        ...
        # Z= discretized dataset
        # Z_encoded = label encoding of Z
        self.encoders_Z = encoders_Z
        self.Z_encoded = encodeDataset(
            self.discreteDataset,
            self.discreteDataset.columns,
            range(0, len(self.discreteDataset.columns)),
            self.encoders_Z,
        )
    ...
    def decodeAttribute_Z(self, value, attribute):
        if attribute in self.encoders_Z:
    ...
    def encodeAttribute_Z(self, value, attribute):
        if attribute in self.encoders_Z:
            if value not in self.encoders_Z[attribute].classes_:
                raise ValueError
            if type(value) not in [np.ndarray, list]:
                value = [value]
            return self.encoders_Z[attribute].transform(value)

        return value

    def encodeInstance_Z(self, instance, features_encode):
        from copy import deepcopy

        instance_encoded = deepcopy(instance)

        for attribute in features_encode:
            instance_encoded[attribute] = self.encodeAttribute_Z(
...
def encodeDataset(df, feature_names, categorical_features_pos, le_encoders):
    from sklearn.preprocessing import LabelEncoder
    from copy import deepcopy
