"""The module provides the LACE_explainer_old class, which is used to perform the
...
from src.LACE_explanation_old import LACE_explanation_old
...
class LACE_explainer_old:
    """The LACE_explainer_old class, through the `explain_instance` method allows
    ...
    def __init__(
        ...
        self.nbrs = sklearn.neighbors.NearestNeighbors(
            ...
            algorithm="auto",
            metric_params=None,
        ).fit(self.train_dataset.X_numpy())
    ...
    def explain_instance(
        ...
        explanation : dict
            ...
            ::
                ...
                {
                    'LACE_explainer_old_o': <src.LACE_explainer_old.LACE_explainer_old object at 0x7fde70c7a828>,
            ...
            * ``"LACE_explainer_old_o"``: A reference to the explainer used to
        ...
        for k in range(self.starting_K, self.max_K, self.K):
            ...
            difference_map, error = self._compute_lace_step(
                encoded_instance,
                k,
    ...
    def _checkCorrectness(
        self, encoded_instance, decoded_target_class, featureMasking=True
    ):
        target_class_index = self.train_dataset.class_values().index(
            decoded_target_class
        )
        ...
        print(allRule)
        return (class_prob - prior_prob[decoded_target_class]) - allRule[
    ...
    def _compute_lace_step(
        ...
        if featureMasking:
            rules = _create_locality_and_get_rules(
                self.train_dataset,
                self.nbrs,
                encoded_instance,
                k,
...
def _create_locality_and_get_rules(
    ...
    if featureMasking:
        ...
        return _create_locality_and_get_rules_OLD(
            training_dataset, nbrs, encoded_instance, k, predict_fn, l3clf
        )
...
def _create_locality_and_get_rules_NEW(
    ...
    y_pred_neighbors = [
        ...
        np.argmax(predict_fn(training_dataset.X().iloc[nearest_neighbors_ixs]), axis=1)
    )
    ...
    # r_attr_ixs_and_values = sorted([l3clf._item_id_to_item[i] for i in r.item_ids])

    # Perform matching: remove all rules that use an attibute value not present in the instance to
    # explain
    # if [r.item_ids for r in encoded_rules]:
    #    print("N", [r.item_ids for r in encoded_rules])
    # [decode_rule(r, l3clf)["body"] for r in encoded_rules]
    # For each rule
    for r in encoded_rules:
        ...
        # https://docs.python.org/3/tutorial/controlflow.html#break-and-continue-statements-and-else-clauses-on-loops
            ...
            # print("Rule", r, decode_rule(r, l3clf))
